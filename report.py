...
def generate_report(site,filename):
    ...
    doc.preamble.append(NoEscape(r'\rfoot{\href{http://hud.coloradoanalytics.com}{http://hud.coloradoanalytics.com}}'))
    ...
    with doc.create(Tabular('c|c|c|c|c|c')) as table:
        table.add_hline()
        table.add_row(('Date','User Name','Growth Rate','Roads DNL','Rail DNL','Airport DNL'))
        table.add_hline()
        table.add_row((current_date.strftime("%B %d, %Y"),site.user_name,percent_str(site.growth_rate) + r'%',site.get_roads_dnl(),site.get_rails_dnl(),site.get_airports_dnl()))
    ...
    if site.roads:
        ...
        for road in site.roads:
            ...
            with doc.create(Tabular('lcccc')) as table:
                ...
                table.add_hline()
                table.add_row(('Autos',num_str(road.auto.adt),percent_str(road.get_auto_adt_fraction()),road.auto.night_fraction,road.auto.speed))
                table.add_row(('Medium Trucks',num_str(road.medium_truck.adt),percent_str(road.get_medium_truck_adt_fraction()),road.medium_truck.night_fraction,road.medium_truck.speed))
                table.add_row(('Heavy Trucks',num_str(road.heavy_truck.adt),percent_str(road.get_heavy_truck_adt_fraction()),road.heavy_truck.night_fraction,road.heavy_truck.speed))
    ...
    if site.airports:
        ...
        for airport in site.airports:
            ...
            with doc.create(Tabular('cc')) as table:
                table.add_row(('Type', 'Distance (miles)'))
                table.add_hline()
                table.add_row((airport.airport_type, feet_to_miles(airport.distance)))
